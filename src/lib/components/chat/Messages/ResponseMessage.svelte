...
{/if}
<!-- Feedback Button -->
<Tooltip content="apertus-feedback@epfl.ch" placement="bottom">
    <a
        href="mailto:apertus-feedback@epfl.ch"
        aria-label="Feedback"
        class="{isLastMessage || ($settings?.highContrastMode ?? false)
            ? 'visible'
            : 'invisible group-hover:visible'} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition inline-flex"
    >
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
